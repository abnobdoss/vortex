...
[[nodiscard]] const vx_array *sample_array() {
    ...
    defer { vx_array_free(height_array); };
    require_no_error(error);
...
UniqueArrayStream sample_array_stream() {
    ...
    for (auto [name, age] : std::views::zip(ages, heights) ) {
        REQUIRE(ArrowArrayAppendInt(arr->children[0], name) == NANOARROW_OK);
        REQUIRE(ArrowArrayAppendInt(arr->children[1], age) == NANOARROW_OK);
...
TEST_CASE("Write file and read dtypes", "[datasource]") {
    ...
    defer { vx_dtype_free(height_dtype); };
...
TEST_CASE("Project single field", "[projection]") {
    ...
    defer { vx_expression_free(age_field); };
    ...
    defer { vx_expression_free(height_field); };
...
TEST_CASE("Scan to Arrow", "[scan]") {
    vx_session *session = vx_session_new();
    defer { vx_session_free(session); };
    TempPath path = write_sample(session, fs::current_path() / "arrow-scan.vortex");
    vx_error *error = nullptr;
